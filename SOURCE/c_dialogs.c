...
HB_FUNC ( SETDLGITEMTEXT )
{
  HG_pstr( lpText ); lpText = HG_parc(3);
   SetDlgItemText ((HWND) HMG_parnl(1), hb_parni(2), lpText ) ; //(LPCTSTR) HMG_parc(3));
   HG_xfree( lpText ) ;
}
...
HB_FUNC( GETDLGITEMTEXT )
{
   ...
   TCHAR * cText = ( TCHAR * ) hb_xgrab( ( nLen + 2 ) * sizeof( TCHAR ) );
   //TCHAR cText [ nLen + 2 ];
...
HB_FUNC ( CHOOSEFONT )
{
   ...
   if ( ChooseFont (&cf) )
   {
      ...
      hb_reta( 8 );
      HG_storvc(lf.lfFaceName, -1, 1, pStr ) ;
...
HB_FUNC ( C_GETFILE )
{
   ...
   HG_pstr( lpsIniDir ); HG_pstr( lpsTitle );HG_pustr( pRetStr ); 
...
//**********************************************************************//   
   ...
   lpsIniDir =  HG_parc(3); lpsTitle  =  HG_parc(2);
      ...
      else
      {
         ...
         if (iNumSelected > 1)
         {
            ...
            for (i = 1; i < iNumSelected; i++) {
              HG_storvc( cFullName[ i ] , -1, i ,pRetStr ) ; }
         } 
         else
          { HG_retc( &buffer[0] ,pRetStr ) ; }
      }
   }
   else
      HMG_retc( _TEXT("") );

HG_xfree( lpsIniDir ) ; HG_xfree( lpsTitle ) ;
...
HB_FUNC ( C_PUTFILE )
{
 ...
 HG_pstr( lpsIniDir ); HG_pstr( lpsTitle );
 HG_pstr( lpsDefFname );HG_pustr( pRetStr );
 HG_pstr( lpsDefExt ) ;
...
//**********************************************************************//   
  lpsDefFname = HG_parc(5); lstrcpy( buffer, lpsDefFname );
  HG_xfree( lpsDefFname ) ;
 ...
 ofn.lpstrFile = (LPTSTR) &buffer;
 ofn.nMaxFile = sizeof(buffer) / sizeof(TCHAR);
 lpsIniDir = HG_parc(3); lpsTitle = HG_parc(2); lpsDefExt = HG_parc(6);
 ofn.lpstrInitialDir = lpsIniDir ;
 ofn.lpstrTitle = lpsTitle ;
 ofn.Flags = flags;
 ofn.lpstrDefExt = lpsDefExt;
...
HG_xfree( lpsIniDir ) ; HG_xfree( lpsTitle ) ; HG_xfree( lpsDefExt ) ;
}
...
// BIF_BROWSEFILEJUNCTIONS: Windows 7 and later. Allow folder junctions such as a library or a compressed file with a .zip file name extension to be browsed.
   ...
   lpsInvldMsg = HG_parc(4);
   BrowseForFolderText.cInvalidDataMsg = lpsInvldMsg ; //(TCHAR *) HMG_parc (4);
   lpsIniDir = HG_parc (5);
   ...
   lpsTitle = HG_parc(1) ;
   BrowseInfo.lpszTitle      = lpsTitle ; //HMG_parc (1);
...
HG_xfree( lpsIniDir ) ; HG_xfree( lpsTitle ) ;HG_xfree( lpsInvldMsg ) ;
