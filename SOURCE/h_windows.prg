...
//#define MCN_FIRST           -750  // ok  i_monthcal.ch
...
*/
   ...
   case nMsg == WM_HOTKEY
    ...
    if _HMG_SYSDATA [4][i] == GetForegroundWindow() //mgmE-737/h_events.prg _HMG_aControlParenthandles
            If _DoControlEventProcedure ( _HMG_SYSDATA [ 6 ] [i] , i )
               Return 0
            EndIf
    endif //20230312 by kenny ooi
         EndIf
...
LOCAL nWidthControl := _HMG_SYSDATA [ 20 ] [i]
   ...
   If r [2] == 1
      ...
      r := ListView_HitTest ( ControlHandle, GetCursorRow() - GetWindowRow (ControlHandle), GetCursorCol() - GetWindowCol (ControlHandle) )
   ElseIf r[ 1 ] > 0 .AND. r[ 2 ] > 0  //20230621 by kenny ooi
