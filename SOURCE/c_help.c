...
HB_FUNC ( WINHELP )
{
	...
	UINT style ;
	BOOL retval;
  HG_pstr( lpHelp ); lpHelp = HG_parc(2);
	switch( n )
	{
	case 0:  style = HELP_FINDER ; context = 0 ; break;
	case 1:  style = HELP_CONTEXT ; context = hb_parni(5) ; break;
	case 2:  style = HELP_CONTEXTPOPUP ; context = hb_parni(5) ; break;
	default: style = HELP_CONTENTS ; context = 0 ;
	}
 
	if (hb_parni(3)) 
      retval = WinHelp( (HWND) HMG_parnl (1), lpHelp , style , context );
	else
      retval = WinHelp( (HWND) HMG_parnl (1), lpHelp , style , context ) ;
